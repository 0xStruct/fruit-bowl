...
const extractStats = (raffle) => {
  const green = "border-drizzle-green"
  const red = "border-red-300"
  const balance = new Decimal(raffle.bowlVault.balance)
  const symbol = raffle.tokenInfo.symbol
  const numberOfWinners = raffle.numberOfWinners ? raffle.numberOfWinners : '???'

  return [
    {
      title: "BOWL Vault Balance",
      content: `${balance.toString()} ${symbol}`,
      color: balance > 0.0 ? red : green
    },
    {
      title: "# of Registrants",
    ...
    {
      title: "Winners (Claimed / Total)",
      content: `${Object.keys(raffle.winners).length} / ${numberOfWinners}`,
      color: green,
      link: null
    }
  ]
...
export default function BowlStatsCard(props) {
