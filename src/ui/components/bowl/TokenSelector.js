...
import { queryBalance } from "../../lib/fruit-scripts"
  ...
  return (
    <div>
      <Combobox as="div" className={props.className} value={props.user && props.user.loggedIn && selectedToken} onChange={async (token) => {
        ...
        <Combobox.Label className="block text-2xl font-flow font-bold">Registration Token<span className="text-red-600">*</span></Combobox.Label>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div className="basis-1/2">
            ...
            <div className="relative">
              <Combobox.Input
                className="w-full h-[46px] text-lg font-flow rounded-2xl border border-drizzle-green bg-drizzle-green-ultralight py-2 pl-3 pr-10  focus:border-drizzle-green-dark focus:outline-none focus:ring-1 focus:ring-drizzle-green-dark"
                onChange={(event) => {
          ...
          <div className="basis-1/2">
            <label className="block text-md font-flow leading-6 mt-2 mb-2">
              # of token required for registration
            </label>
            <input
              type="number"
              value={wagerAmount}
              id="wagerAmount"
              className="grow w-full h-[46px] rounded-2xl focus:ring-drizzle-green-dark focus:border-drizzle-green-dark bg-drizzle-green-ultralight border-drizzle-green font-flow text-lg placeholder:text-gray-400"
              onWheel={(e) => e.target.blur()}
              onChange={(event) => { setWagerAmount(event.target.value); props.onWagerAmount(event.target.value); }}
            />
          </div>
        </div>
      </Combobox>
