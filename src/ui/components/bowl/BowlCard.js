...
const MemoizeBasicInfo = React.memo(({ nameService, host, createdAt, token, eligibilityMode, raffle }) => {
  return (
...
export default function BowlCard(props) {
  ...
  const { raffle, claimStatus, user, token,
  ...
  const tokenInfo = (raffle && raffle.tokenInfo) || props.token
  const wagerAmount = (raffle && raffle.wagerAmount) || props.wagerAmount
  const wagerOptions = (raffle && raffle.wagerOptions) || props.wagerOptions

  const [selectedWagerOption, setSelectedWagerOption] = useState(null)
  const [selectedWagerOptionText, setSelectedWagerOptionText] = useState(null)
  const onSelectedWagerOption = e => {
    setSelectedWagerOption(e.target.value)

    const optionIndex = Number(e.target.value) - 1
    setSelectedWagerOptionText(fruits[optionIndex] +' '+ wagerOptions.split("\n")[optionIndex])
  }

  /*
  useEffect(() => {
    console.log(selectedWagerOption)
    console.log(selectedWagerOptionText)
  }, [selectedWagerOption])*/
    ...
    <div className="w-full justify-center flex flex-col gap-y-8 mt-2 mb-2 sm:flex-row sm:gap-x-8 text-black">
      ...
      md:w-[480px]">
        ...
        <div className="flex flex-col p-5 sm:p-8 gap-y-5">
          ...
          <MemoizeBasicInfo nameService={nameService}
            host={host} createdAt={createdAt} token={token} eligibilityMode={eligibilityMode} raffle={raffle}
          />
          ...
          <p>{`🎟️ ${wagerAmount} ${tokenInfo.symbol} tokens per registration`}</p>

          {/*<MemoizeWagerOptions wagerOptions={wagerOptions} />*/}
          <div className="w-full mb-4">
            <b>Outcomes (Pick 1)</b>
            <p className="w-full font-flow text-base break-words whitespace-pre-wrap mt-2">
              <div>
              {wagerOptions.split("\n").map((option, i) => (
                <div className="p-2">
                <label className="p-2 hover:bg-pink-100 text-xl w-full">
                  <input type="radio" value={i+1} className="p-2 mr-2" 
                    checked={selectedWagerOption === (i+1).toString()} 
                    onChange={onSelectedWagerOption}
                    text="aaa"
                  />{fruits[i]}{" "}{option}
                </label>
                </div>
      ...
      <div className="flex flex-col gap-y-8">
        ...
        <ActionCard
          ...
          host={host}
          token={token}
          selectedWagerOption={selectedWagerOption}
          selectedWagerOptionText={selectedWagerOptionText}
          //nft={nft}
          user={user}
