...
import { register, claim } from "../../lib/fruit-transactions"
...
const parseClaimStatus = (user, claimStatus, displayName, isPreview) => {
  const elements = {
    emoji: "🎉", description: "YOU WON", amount: `???`, title: "PREVIEWING"
  }
  ...
  let eStatusR = claimStatus.eligibilityForRegistration.status.rawValue
  let eStatusC = claimStatus.eligibilityForClaim.status.rawValue
  let eligibleNFT = '' //claimStatus.eligibilityForClaim.eligibleNFTs[0] // @struct
  ...
  if (aStatus === "1") {
    ...
    if (eStatusR === "0") {
      elements.emoji = "✅"
      elements.description = "ELIGIBLE"
      elements.title = "REGISTER NOW"
      elements.amount = null
    } else if (eStatusR === "2") {
      elements.emoji = "🙉"
      elements.description = "NOT ELIGIBLE"
      elements.title = "NOT ELIGIBLE"
      elements.amount = null
    } else if (eStatusR === "4") {
      elements.emoji = "✅"
      elements.description = "REGISTERED"
      elements.title = "COME CHECK LATER"
      elements.amount = null
    }  
  }
...
export default function ActionCard(props) {
  ...
  const { isPreview, raffle, host, user, nft, token, selectedWagerOption, selectedWagerOptionText, claimStatus, setShowClaimedModal, setShowRegisteredModal, setRewardInfo } = props
  ...
  shadow-drizzle
  ">
        <label className="block w-full text-center text-[34px]">{emoji}</label>
        ...
        {claimStatus?.availability.status.rawValue === "2" && claimStatus?.eligibilityForRegistration.status.rawValue === "0" && 
          <label className="block w-full text-center">{`Pick: ${selectedWagerOptionText}`}</label>
        }
        {claimStatus?.availability.status.rawValue === "2" && claimStatus?.eligibilityForRegistration.status.rawValue === "4" && 
          <label className="block w-full text-center">{`Picked: ${fruits[raffle.registrationRecords[user.addr]?.wagerOption]}`}</label>
        }
        <button
          type="button"
          ...
          onClick={async () => {
            ...
            // Registering
            // @struct register (pick and pay)
            if (claimStatus.availability.status.rawValue == "2" && claimStatus?.eligibilityForRegistration.status.rawValue === "0") {
              if(!selectedWagerOption) { alert("pick an option first"); return;}
              // @struct pick and pay
              const res = await register(raffle, selectedWagerOption,
                setTransactionInProgress,
                setTransactionStatus)

              if (res && res.status === 4 && res.statusCode === 0) {
                //console.log(res.events)
                const event = res.events.find((e) => e.type.includes("BowlRegistered"))
                if (event) {
