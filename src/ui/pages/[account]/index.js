...
//import RaffleList from '../../components/bowl/RaffleList'
//import { queryRaffles } from '../../lib/mist-scripts'

import BowlList from '../../components/bowl/BowlList'
import { queryBowls } from '../../lib/fruit-scripts'
...
import EnrolledHistory from '../../components/bowl/EnrolledHistory'
...
const rafflesFetcher = async (funcName, address) => {
  ...
  return await queryBowls(address)
}
...
const extractRecords = (records) => {
  ...
  for (const [type, typeRecords] of Object.entries(records)) {
    ...
    } else if (type.includes("FruitBowl")) {
      raffleRecords = Object.values(typeRecords).sort((a, b) => {
      return parseInt(b.bowlID) - parseInt(a.bowlID)
    })
  }
  }
  ...
  const showList = () => {
    ...
    if (showRaffle) {
      if (!rafflesData) {
        ...
        return (
          <div className="mt-10 flex flex-col gap-y-10">
            <BowlList bowls={raffles} user={props.user} pageAccount={username} />
            {/*
